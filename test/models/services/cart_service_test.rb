...
class CartServiceTest < ActiveSupport::TestCase
  ...
  test '加入購物車' do
    CartService.add_product_to_cart @book, @cart
  ...
  test '建立訂單後清空購物車' do
    CartService.create_order_from_cart @cart, @order_params
  ...
  test '已付款的訂單無法建立交易' do
    ...
    assert_raises CartService::OrderIsPaid do
      CartService.create_transaction_from_order order
    end
  end
end